...
class SwerveDrive:
    ...
    def setup(self):
        ...
        self.width = (19.5/12)/2 #take witdth in inches convert it to feet and half it
        self.length = (22 / 12)/2 #same as above ^^^ but with length

        self.request_wheel_lock = False #setting breaking mode to false
    ...
    def square_input(input):
        return math.copysign(input * input, input) # return magintude of x but the sign of y
    ...
    def flush(self):
        #resets all requested values of the drive system
        #it will reset each module individaually
        self._requested_vectors = {
            'fwd': 0,
            'strafe': 0,
            'rcw': 0
    ...
    def set_raw_fwd(self, fwd):
        # sets the raw value between -1 to 1
    ...
    def _calculate_vectors(self):
        self._requested_vectors['fwd'],self._requested_vectors['strafe'], self._requested_vectors['rcw'] = self.normailze([self._requested_vectors['fwd'],self._requested_vectors['strafe'], self._requested_vectors['rcw']])
        ...
        if self._requested_vectors['rcw'] == 0 and self._requested_vectors['strafe'] == 0 and self._requested_vectors['fwd'] == 0: # prevents runtime error
            self._requested_speeds = dict.fromkeys(self._requested_speeds, 0) #dont reset the wheel angles

            if self.request_wheel_lock:
                #puts wheels in a way that its difficult to push the robot with the intent for defence
        ...
        # get speed and angle when given the corresponding quad vectors
    ...
    def execute(self):
        #sends the speeds and angles to the wheel modules then excutes the do it in each wheel module
        self.update_smartdash()

        self._calculate_vectors()

        #set speed and angles for each one of the modules
        for key in self.modules:
